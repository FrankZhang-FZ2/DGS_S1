---
title: "DGS_S1_analyses_Noutliers"
author: "Qilin Zhang"
date: "2023-04-13"
...
#cleaning based on completion time
...
for(i in 1:7){
  percent_i <- function(x){
  sum((x==i)/length(x)*100)
  }
  number_i <- apply(data,1,percent_i)
  data <- data[c(number_i<90),]
}
...
```#{r outliers}
scale_vars<- data[,-c(grep("Age",colnames(data)):ncol(data))] %>%
  select(-c(DGS_31,DGS_53))

mahal <- mahalanobis(scale_vars,
                     colMeans(scale_vars, na.rm = TRUE),
                     cov(scale_vars),use = "pairwise.complete.obs")
mahal

cutoff <- qchisq(p = 1-0.001, # 1 minus alpha
                 df = ncol(scale_vars))
table(mahal < cutoff)


data <- data[c(mahal < cutoff),]
...
EFA_fit_8 <- fa(DGS_fit_8,
             nfactors = 8,
             rotate = "oblimin",
             fm="ml")
EFA_fit_8 <- fa.sort(EFA_fit_8)
EFA_fit_8
print(EFA_fit_8$loadings, cutoff = 0.3)
print(EFA_fit_8$loadings) #model Abandoned because of conceptual reasons (see write-up)
